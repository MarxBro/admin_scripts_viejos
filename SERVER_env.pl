...
if ($info{'APACHE_LIB'}) {
  my @standard_mod = ('mod_rewrite.so', 'mod_cgi.so', 'mod_perl.so', 'mod_env.so', 'mod_imap.so', 'mod_include.so', 'mod_alias.so', 'mod_access.so', 'mod_browser.so', 'mod_python.so', 'mod_unique_id.so');
  my @auth_mod     = ('mod_auth.so', 'mod_digest.so', 'mod_auth_anon.so', 'mod_auth_db.so', 'mod_auth_dbm.so', 'mod_auth_cookie.so', 'mod_auth_digest.so', 'mod_auth_mysql.so');
  my @ext_mod      = ('mod_expires.so', 'mod_fastcgi.so', 'mod_gzip.so', 'mod_headers.so', 'libphp3.so', 'libphp4.so', 'mod_proxy.so', 'mod_speling.so', 'mod_status.so',  'mod_usertrack.so', 'mod_vhost_alias.so');
...
elsif (-e "/usr/lib/apache2/mod_env.so") {$info{'APACHE_LIB'} = "/usr/lib/apache2"}
